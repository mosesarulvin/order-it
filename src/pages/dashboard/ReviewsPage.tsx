  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-0.5">Customer feedback for your shop</p>
      </div>

      {/* Summary stats */}
      {reviews.length > 0 && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-5 flex flex-col sm:flex-row gap-6">
          {/* Average */}
          <div className="flex flex-col items-center justify-center min-w-[100px] gap-1">
            ...
            <span className="text-xs text-gray-400 dark:text-gray-500 mt-1">{reviews.length} review{reviews.length !== 1 ? 's' : ''}</span>
          </div>
          ...
          <div className="flex-1 space-y-1.5">
            {ratingCounts.map(({ rating, count }) => (
              <button
                ...
                className={`flex items-center gap-2 w-full group rounded-lg px-2 py-1 transition-colors ${filterRating === rating ? 'bg-amber-50 dark:bg-amber-900/30' : 'hover:bg-gray-50 dark:hover:bg-slate-800'}`}
              >
                <span className="text-xs font-medium text-gray-500 dark:text-gray-400 w-3">{rating}</span>
      ...
      {filterRating !== null && (
        <div className="flex items-center gap-2">
          <span className="text-sm text-gray-600 dark:text-gray-300">Showing {filterRating}-star reviews</span>
          <button
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((review) => (
            <div key={review.id} className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-4 space-y-2">
              <div className="flex items-start justify-between">
                <div>
                  <p className="font-semibold text-gray-900 dark:text-white text-sm">{review.customer_name}</p>
                  {review.order_number && (
                    <p className="text-xs text-gray-400 dark:text-gray-500">Order #{review.order_number}</p>
                  )}
                </div>
