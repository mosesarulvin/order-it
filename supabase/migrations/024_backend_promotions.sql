...
-- Helper function to calculate discounted price based on active promotions
...
AS $$
DECLARE
  v_best_price NUMERIC := p_original_price;
  v_current_price NUMERIC;
  v_promo RECORD;
BEGIN
  FOR v_promo IN
    SELECT discount_type, discount_value, target_type, target_id
    FROM promotions
    WHERE shop_id = p_shop_id
      AND is_active = true
      AND (valid_until IS NULL OR valid_until > now())
  LOOP
    IF v_promo.target_type = 'all' OR 
       (v_promo.target_type = 'category' AND v_promo.target_id = p_category_id) OR
       (v_promo.target_type = 'item' AND v_promo.target_id = p_menu_item_id) THEN
       
      IF v_promo.discount_type = 'percentage' THEN
        v_current_price := p_original_price * (1 - v_promo.discount_value / 100.0);
      ELSIF v_promo.discount_type = 'flat' THEN
        v_current_price := p_original_price - v_promo.discount_value;
      END IF;
      
      v_current_price := GREATEST(0, v_current_price);
      
      IF v_current_price < v_best_price THEN
        v_best_price := v_current_price;
      END IF;
    END IF;
  END LOOP;
  
  RETURN ROUND(v_best_price, 2);
END;
$$;
